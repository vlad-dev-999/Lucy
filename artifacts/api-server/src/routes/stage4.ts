...
router.put("/canonical-items/:canonicalItemId/common-use-mmf", async (req, res, next) => { try { const input = mmfInput.safeParse(req.body); if (!input.success) return invalid(res, input); const [canonical] = await db.select({ id: canonicalItemsTable.id }).from(canonicalItemsTable).where(eq(canonicalItemsTable.id, req.params.canonicalItemId)).limit(1); if (!canonical) return res.status(404).json({ error: "Canonical item not found" }); const values = { currentDglpMmf: input.data.dglpMmf ?? null, currentEchsMmf: input.data.echsMmf ?? null, updatedBy: req.get("x-user-id")?.trim() || "demo-hospital-user", updatedAt: new Date() }; const [row] = await db.insert(commonUseMmfTable).values({ id: `common_use_${crypto.randomUUID()}`, canonicalItemId: canonical.id, ...values }).onConflictDoUpdate({ target: commonUseMmfTable.canonicalItemId, set: values }).returning(); res.json(row); } catch (error) { next(error); } });
